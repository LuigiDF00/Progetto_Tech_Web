...
export default function RiddlesPage() {
  ...
  const sortedRiddles = [...riddles].sort((a, b) => (b.solved_by_count || 0) - (a.solved_by_count || 0));
  ...
  const getTierInfo = (riddleId: number) => {
    const rankIndex = top3Ids.indexOf(riddleId);
    if (rankIndex === 0) {
      return { name: 'ORO', tierClass: 'tier-gold', badgeClass: 'gold', boxClass: 'gold-bg', btnClass: 'card-btn-gold', Icon: PixelTrophyGold };
    } else if (rankIndex === 1) {
      return { name: 'ARGENTO', tierClass: 'tier-silver', badgeClass: 'silver', boxClass: 'silver-bg', btnClass: 'card-btn-silver', Icon: PixelTrophySilver };
    } else if (rankIndex === 2) {
      return { name: 'BRONZO', tierClass: 'tier-bronze', badgeClass: 'bronze', boxClass: 'bronze-bg', btnClass: 'card-btn-bronze', Icon: PixelTrophyBronze };
    }
    return null;
  };
      ...
      {loading ? (
        ...
        <div className="cards-grid">
          {filteredRiddles.map((riddle) => {
            const tier = getTierInfo(riddle.id);
            const isSolved = Number(riddle.is_solved_by_current_user) === 1;
            const exampleLines = getExampleLines(riddle);

            return (
              <div key={riddle.id} className={`riddle-card ${tier ? tier.tierClass : 'tier-standard'}`}>
                {/* Top Notch Badge */}
                {tier && (
                ...
                <div>
                  ...
                  <Link to={`/riddles/${riddle.id}`} className={`card-btn ${tier ? tier.btnClass : 'card-btn-standard'}`}>
